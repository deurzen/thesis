...
#define TRY_OR_CONTINUE(smt) \
    ...
    dds::pub::DataWriter<GlobalState> writer = nullptr;
    ...
    while (!application::shutdown_requested) {
        ...
        if (!may_participate && writer_online) {
            ...
            writer = dds::pub::DataWriter<GlobalState>(publisher, topic);
            writer_online = true;
        ...
        if (may_participate) {
            ...
            if (commrounds_differ) {
                ...
                writer.write(data);
                writer->wait_for_asynchronous_publishing(sleep_duration);
            } else
                std::cout << "Not republishing state for commround " << commround << std::endl;
        }
