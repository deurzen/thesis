...
#define TRY_OR_CONTINUE(smt) \
    ...
    dds::topic::Topic<GlobalState> topic(participant, "Global State");

    dds::sub::Subscriber subscriber(participant);
    dds::sub::DataReader<GlobalState> reader = nullptr;
    ...
    auto data_handler = [&reader,&may_participate,server_host,server_port]() {
        ...
        dds::sub::LoanedSamples<GlobalState> samples = reader.take();
        ...
        for (auto const &sample : samples) {
            ...
            if (sample.info().valid()) {
                ...
                std::string commround = sample_data.commround();
                std::string state;
                ...
                std::string data = "commround=" + commround;
                data += "&state=" + state;

                std::cout << "Received global state" << std::endl;
    ...
    while (!application::shutdown_requested) {
        ...
        if (!may_participate && reader_online) {
            ...
            reader = dds::sub::DataReader<GlobalState>(subscriber, topic);
