...
#define TRY_OR_CONTINUE(smt) \
    ...
    std::vector<std::string> filter_params = { server_id };
    dds::topic::Filter filter("NOT id MATCH %0", filter_params);

    filter->name(rti::topic::stringmatch_filter_name());

    auto cft = dds::topic::ContentFilteredTopic<State>(
        topic,
        "OwnIdFilter",
        filter
    );
    ...
    auto data_handler = [&reader,&may_participate,server_host,server_port]() {
        reader.wait_for_historical_data(dds::core::Duration(7200));
    ...
    while (!application::shutdown_requested) {
        try {
            ...
            if (next_server_id != server_id) {
                ...
                cft->append_to_expression_parameter(0, server_id);
        ...
        if (!may_participate && reader_online) {
            ...
            reader = dds::sub::DataReader<State>(subscriber, cft);
